...
Plain web games, Emscripten C/C++ ports, LÖVE (love.js), Godot 3 and 4, Unity WebGL, Flash, DOS and retro consoles are supported. Multithreaded builds of any of these need Quantiverse served over http(s) rather than opened from disk. See [docs/ENGINES.md](docs/ENGINES.md) for the full compatibility table and each engine's requirements.
...
- **`requires`:** `["crossOriginIsolated"]` for multithreaded games. QuantiVerter sets it.
...
- [brotli](https://www.npmjs.com/package/brotli) 1.3.3 decompressor (MIT), used to unpack Brotli-compressed Unity builds.
