...
open(p, "w").write(s)
PY
# Save file in $HOME (kept in IndexedDB by persist.js) rather than the working directory.
emcc -O2 -DSFG_SAVE_FILE_PATH='"/home/web_user/anarch.sav"' main_sdl.c -o "$OUT/index.html" $QV_LINK -sUSE_SDL=2 -lopenal -sEXPORTED_FUNCTIONS='["_main","_webButton"]' -sEXPORTED_RUNTIME_METHODS='["ccall","cwrap"]'
